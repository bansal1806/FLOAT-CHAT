...
//                       ? 'bg-aqua-500/20 border border-aqua-500/50' 
...
// import { realtimeDataService, ArgoFloatData } from '@/lib/realtimeData'
// import { 
//   MapPin, Thermometer, Waves, Eye,
//   RotateCcw, ZoomIn, ZoomOut, Play, Pause,
//   Download, BarChart3, Globe, Layers, Settings,
//   RefreshCw, Filter, Search, Activity, TrendingUp,
...
// // Load Earth model using useGLTF
...
//     inactive: <Activity className="w-3 h-3" />,
...
//         <meshStandardMaterial
//           color={statusColors[float.status] || '#00aaff'}
//           emissive={statusColors[float.status] || '#00aaff'}
//           emissiveIntensity={0.6}
//         />
//       </mesh>
//       {hovered && (
//         <Html distanceFactor={10} occlude>
//           <div className="bg-black bg-opacity-70 text-white rounded-md p-3 text-xs w-48 shadow-lg pointer-events-none">
//             <div className="font-semibold">ARGO Float {float.id}</div>
//             <div>{statusIcons[float.status]} {float.status.toUpperCase()}</div>
//             <div>Temperature: {float.temperature} °C</div>
//             <div>Salinity: {float.salinity} PSU</div>
//             <div>Depth: {float.depth} m</div>
//             <div>Battery: {float.battery} %</div>
//             <div>Last update: {float.lastUpdate}</div>
//           </div>
//         </Html>
//       )}
//     </group>
...
//       </group>
//       {filteredFloats.map(float => (
//         <ArgoFloatMarker key={float.id} float={float} onClick={() => setSelectedFloat(float)} />
//       ))}
//       <OrbitControls enablePan enableZoom enableRotate minDistance={1.5} maxDistance={8} />
//     </>
//   )
// }
...
//                     onClick={() => setIsRealtime(!isRealtime)}
//                     className={`p-2 rounded-lg transition-colors ${
//                       isRealtime ? 'bg-aqua-500 text-white' : 'bg-deep-700 text-gray-400 hover:text-white'
//                     }`}
//                     title="Real-time Data"
//                   >
//                     <Activity className={`w-4 h-4 ${isRealtime ? 'animate-pulse' : ''}`} />
...
//                     onClick={() => setAutoRefresh(!autoRefresh)}
//                     className={`p-2 rounded-lg transition-colors ${
//                       autoRefresh ? 'bg-green-500 text-white' : 'bg-deep-700 text-gray-400 hover:text-white'
//                     }`}
//                     title="Auto Refresh"
//                   >
//                     <RefreshCw className={`w-4 h-4 ${autoRefresh ? 'animate-spin' : ''}`} />
...
//                     onClick={() => setIsRotating(!isRotating)}
//                     className={`p-2 rounded-lg transition-colors ${
//                       isRotating ? 'bg-green-500 text-white' : 'bg-deep-700 text-gray-400 hover:text-white'
//                     }`}
//                     title="Toggle Rotation"
//                   >
//                     {isRotating ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
...
//             {selectedFloat ? (
//               <div className="space-y-4">
//                 {/* Details content same as your original */}
//               </div>
//             ) : (
//               <div className="text-center text-gray-400 py-8">
//                 <Eye className="w-12 h-12 mx-auto mb-3 opacity-50" />
...
//           {/* Global Stats, Float List, Quick Actions ... place your original UI here */}
//         </div>
//       </div>
//     </div>
//   )
...
// Your full mock data from previous snippet
const mockArgoFloats: ArgoFloat[] = [
  { id: 'A1', lat: 20, lon: 60, temperature: 28.5, salinity: 35.2, depth: 1000, lastUpdate: '2 min ago', status: 'active', region: 'Arabian Sea', battery: 87, mission: 'Temperature Monitoring', dataQuality: 'excellent', oxygen: 4.2, pressure: 100.5, ph: 8.1 },
  { id: 'A2', lat: -10, lon: 120, temperature: 26.8, salinity: 34.9, depth: 1500, lastUpdate: '5 min ago', status: 'active', region: 'Pacific Ocean', battery: 92, mission: 'Climate Research', dataQuality: 'excellent', oxygen: 3.8, pressure: 150.2, ph: 8.0 },
  { id: 'A3', lat: 35, lon: -120, temperature: 18.2, salinity: 35.8, depth: 2000, lastUpdate: '1 min ago', status: 'warning', region: 'North Pacific', battery: 45, mission: 'Deep Water Study', dataQuality: 'good', oxygen: 2.1, pressure: 200.8, ph: 7.9 },
...
// Earth GLTF model loader component
function EarthModel(props: JSX.IntrinsicElements['group']) {
  const { scene } = useGLTF('/models/earth.glb')
  return <primitive object={scene} {...props} />
}

// ARGO float marker component with hover tooltip
function ArgoFloatMarker({ float, onClick }: { float: ArgoFloat; onClick: () => void }) {
  const meshRef = useRef<THREE.Mesh>(null)
  const [hovered, setHovered] = useState(false)
    ...
    <group position={[x, y, z]}>
      ...
      {hovered && (
        <Html distanceFactor={8} occlude>
          <div className="bg-black bg-opacity-70 text-white rounded-md p-3 text-xs shadow-lg pointer-events-none min-w-[220px]">
            <div className="font-semibold">ARGO Float {float.id}</div>
            <div className="flex items-center gap-1 my-1">
              {statusIcons[float.status]}
...
// Core Ocean Globe 3D component with Earth model and ARGO floats
function OceanGlobe3D({
  filteredFloats,
  selectedFloat,
  setSelectedFloat,
  ...
  filteredFloats: ArgoFloat[]
  selectedFloat: ArgoFloat | null
  setSelectedFloat: React.Dispatch<React.SetStateAction<ArgoFloat | null>>
  isRotating: boolean
}) {
  const earthRef = useRef<THREE.Group>(null!)
...
// Main app component with filter, search, panels, and real-time data
export default function OceanGlobe() {
  const [selectedFloat, setSelectedFloat] = useState<ArgoFloat | null>(null)
  const [isRotating, setIsRotating] = useState(true)
  const [filterStatus, setFilterStatus] = useState<'all' | 'active' | 'warning' | 'error'>('all')
  ...
  // Convert real-time data to ArgoFloat type
  const convertToArgoFloat = (data: ArgoFloatData): ArgoFloat => ({
    id: data.id,
    lat: data.lat,
    lon: data.lon,
  ...
  // Choose floats from real-time or mock
  const currentFloats =
    isRealtime && realtimeFloats.length > 0
      ? realtimeFloats.map(convertToArgoFloat)
      : mockArgoFloats
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3">
          <div className="bg-deep-800/50 backdrop-blur-sm border border-deep-700 rounded-xl overflow-hidden">
            <div className="bg-gradient-to-r from-ocean-500/20 to-aqua-500/20 border-b border-deep-700 p-4 flex justify-between items-center">
              <div>
                <h2 className="text-xl font-semibold text-white">3D Ocean Globe</h2>
        ...
        {/* Sidebar Section: Detailed float info, stats, list, quick actions */}
        <div className="space-y-6">
          {/* Selected Float Info */}
          <div className="bg-deep-800/50 backdrop-blur-sm border border-deep-700 rounded-xl p-4">
            <h3 className="text-lg font-semibold text-white mb-4">Float Details</h3>
